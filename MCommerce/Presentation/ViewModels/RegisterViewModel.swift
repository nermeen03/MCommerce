...
class RegisterViewModel : ObservableObject {
    ...
    var errorMessage: String {
        var errors: [String] = []
        if !emailError.isEmpty { errors.append(emailError) }
        if !passwordError.isEmpty { errors.append(passwordError) }
        if !confirmPasswordError.isEmpty { errors.append(confirmPasswordError) }
        if !nameError.isEmpty { errors.append(nameError) }
        if !phoneNumberError.isEmpty { errors.append(phoneNumberError) }
